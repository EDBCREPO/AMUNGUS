...
//  minigame_custom
    ...
    node_t minigame_tmp() { return node_t([=]( ptr_t<node_t> self ){
        ...
        auto tmp  = ptr_t<uchar64>( 0UL );
        auto uif  = fs::read_file( "assets/html/minigame/navigation.html" );
        ...
        self->onNext ([=](){ if( key::is_pressed( rl::KEY_ESCAPE ) ){ self->free(); } });
        self->onClose([=](){ 
        ...
        uif.then([=]( string_t data ){
            ...
            EM_EVAL( _STRING_(
                ...
                document.querySelector( '[game-modal]' ).append( ol );
    ...
    node_t minigame_scan() { return node_t([=]( ptr_t<node_t> self ){

        auto vpt  = self->get_root_viewport(); 
        auto tmp  = ptr_t<uchar64>( 0UL );
        auto obj  = *self->get_root()->get_attribute<object_t>("task");
        auto uif  = fs::read_file( "assets/html/minigame/scan.html" );
        ...
        uif.then([=]( string_t data ){
            ...
            self->get_root()->onSignal.emit( "ply_tp", obj["pos"].as<vec3_t>()*vec3_t({1,0,1}) );
            auto processed = regex::format( data, NODEPP_MODULE_NAME, addr, "20" );
            matrix::set_pixel_float( vpt->matrix, 11, 0, 0, 1. );

            self->onNext.add( coroutine::add( COROUTINE(){
            coBegin
                coDelay(20000); self->free();
            coFinish
            }));
            ...
            self->append_child( node::node_sphere([=]( ptr_t<node_t> self ){
                ...
                pos->position = obj["pos"].as<vec3_t>() + vec3_t({ 1, 0, 0 });
                pos->scale    = vec3_t({ 5, 1, 5 });

                self->onLoop([=]( float delta ){
                    pos->position.y = abs( sin( *stm ) * 12 );
                *stm += delta; });
    ...
    node_t minigame_vent() { return node_t([=]( ptr_t<node_t> self ){

        auto tmp = ptr_t<uchar64>(0UL); auto ppt = ptr_t<char>(0UL,0x00);
        auto obj = *self->get_root()->get_attribute<object_t>("vent");
        auto uif = fs::read_file( "assets/html/minigame/vent.html" );
        auto vpt = self->get_root_viewport(); 
        auto flg = matrix::get_pixel_float( vpt->matrix, 11, 0, 0 );

        ppt[0]   = obj["c"].as<int>();
        auto len = obj["vent"].as<array_t<object_t>>().size();

        auto addr = process::invoke([=]( any_t raw ){
        auto value= raw.as<EM_VAL>(); switch( value.as<int>() ){

            case 1: do { ppt[0]--; if( ppt[0]<0 ) { ppt[0]=len-1; }
            auto v= obj["vent"][ppt[0]]; int c=2; do {
                self->get_root()->onSignal.emit( "ply_tp", v.as<vec3_t>() );
            } while(c-->0); } while(0); break;
            
            case 2: do { ppt[0]++; if( ppt[0]>=len ) { ppt[0]=0; }
            auto v= obj["vent"][ppt[0]]; int c=2; do {
                self->get_root()->onSignal.emit( "ply_tp", v.as<vec3_t>() );
            } while(c-->0); } while(0); break;
        ...
        self->onNext([=](){
            if( key::is_pressed( 'A' ) ){ process::call( addr, EM_VAL(1) ); }
            if( key::is_pressed( 'D' ) ){ process::call( addr, EM_VAL(2) ); }
            if( key::is_pressed( rl::KEY_ESCAPE ) ){ self->free(); }
        });
    ...
    node_t minigame_camera() { return node::node_3D([=]( ptr_t<node_t> self ){
        ...
        auto obj = (*self->get_root()->get_attribute<object_t>( "object" ))["camera"];
        auto tmp = ptr_t<uchar64>(0UL); auto ppt = ptr_t<char>(0UL,0x00);
        auto vpt = self->get_root_viewport(); 
        auto len = obj.as<array_t<object_t>>().size();
        auto flg = matrix::get_pixel_float( vpt->matrix, 11, 0, 0 );
        ...
        auto uif = fs::read_file( "assets/html/minigame/camera.html" );
        ...
        auto value= raw.as<EM_VAL>(); switch( value.as<int>() ){
            ...
            case 1: do { ppt[0]--; if( ppt[0]<0 ){ ppt[0]=len-1; }
                auto pos = self->get_child("cam3D")->get_attribute<transform_3D_t>("transform");
                     pos->rotation = obj[ppt[0]][1].as<vec3_t>(); onName.emit(ppt[0]);
                     pos->position = obj[ppt[0]][0].as<vec3_t>(); 
            } while(0); break;
        ...
        uif.then([=]( string_t data ){
            ...
            self->append_child( "cam3D", node::node_camera_3D([=]( ptr_t<node_t> self ){
                ...
                auto stm = ptr_t<float>( 0UL ); cam->fovy = 140;

                /*FIXME: cam fovy screen size based*/

                pos->rotation = obj[0][1].as<vec3_t>(); onName.emit(0);
                pos->position = obj[0][0].as<vec3_t>(); 
    ...
    node_t minigame_card() { return node_t([=]( ptr_t<node_t> self ){
        ...
        auto uif  = fs::read_file( "assets/html/minigame/admin.html" );
        auto val  = ptr_t<int>( 0UL, rand() % 17 * 100 + 300 );
        auto tmp  = ptr_t<uchar64>( 0UL ); 
    ...
    node_t minigame_password() { return node_t([=]( ptr_t<node_t> self ){
        ...
        auto tmp  = ptr_t<uchar64>( 0UL ); 
        auto que  = queue_t<int>();
        auto uif  = fs::read_file( "assets/html/minigame/password.html" );
        ...
        auto value= raw.as<EM_VAL>(); switch( value.as<int>() ){
            ...
            ),  array_t<int>( que.data() ).join(""), pss.join(""), *tmp ); 
            } while(0); break;
    ...
    node_t minigame_wiring() { return node_t([=]( ptr_t<node_t> self ){
        ...
        auto mpos= self->get_root()->get_attribute<vec3_t>("mousePosition");
        auto tmp = ptr_t<uchar64>( 0UL ); auto spos = ptr_t<vec4_t>(0UL);
        auto uif = fs::read_file( "assets/html/minigame/wiring.html" );
        ...
        auto value= raw.as<EM_VAL>(); switch( value.as<int>() ){
            ...
            case 0x01: case 0x02: case 0x03: case 0x04: case 0x05: do {
                ...
                ),  *tmp, val[val[10]] ).as<bool>(); if(!has) { break; }
                ...
                ),  *tmp, val[10] ); } val[10] = value.as<int>();
            ...
            case 0x10: case 0x20: case 0x30: case 0x40: case 0x50: do {
                ...
                if(!el){ return; }
                    el.setAttribute( "x2", el.getAttribute( "x1" ) );
                    el.setAttribute( "y2", el.getAttribute( "y1" ) );
                ),  *tmp, val[10] ); val[10] = 0x00; break; }

                EM_EVAL( _STRING_(
                    document.querySelector("[_${0}_pow]").setAttribute( "c${1}", "" );
                ),  *tmp, val[raw+4] ); val[10] = 0x00;
        ...
        uif.then([=]( string_t data ){
            ...
            self->onLoop.add([=]( float delta ){ return co.emit(); });
    ...
    node_t minigame_reactor() { return node_t([=]( ptr_t<node_t> self ){
        ...
        auto tmp = ptr_t<uchar64>( 0UL );
        auto uif = fs::read_file( "assets/html/minigame/reactor.html" );
    ...
    node_t minigame_gas() { return node_t([=]( ptr_t<node_t> self ){
        ...
        auto tmp  = ptr_t<uchar64>( 0UL ); auto done = ptr_t<bool>(0UL,false);
        auto uif  = fs::read_file( "assets/html/minigame/gasoline.html" );
        ...
        auto value= raw.as<EM_VAL>(); switch( value.as<int>() ){
            ...
            ), *tmp ); self->onNext.add( coroutine::add( COROUTINE(){
            coBegin
    ...
    node_t minigame_simon() { return node_t([=]( ptr_t<node_t> self ){
        ...
        auto tmp  = ptr_t<uchar64>( 0UL ); auto enb = ptr_t<char>( 5UL, 0x00 );
        auto uif  = fs::read_file( "assets/html/minigame/simon.html" );
    ...
    node_t minigame_nav( array_t<string_t> emj ) { return node_t([=]( ptr_t<node_t> self ){
        ...
        self->onNext ([=](){ 
            if( key::is_pressed( rl::KEY_ESCAPE ) ){ self->free(); } 
            if( key::is_pressed( rl::KEY_A ) ){ process::call( addr, EM_VAL(3) ); }
            if( key::is_pressed( rl::KEY_D ) ){ process::call( addr, EM_VAL(4) ); }
        });
    ...
    node_t minigame_electricity() { return node_t([=]( ptr_t<node_t> self ){
        ...
        auto uif = fs::read_file( "assets/html/minigame/switch.html" );
        auto tmp = ptr_t<uchar64>( 0UL );
    ...
    node_t minigame_radio() { return node_t([=]( ptr_t<node_t> self ){
        ...
        auto scl = 10. / 100.; auto tmp = ptr_t<uchar64>( 0UL );
        auto val = ptr_t<int>   ({ rand() % 30 + 3, rand() % 30 + 3 });
        auto uif = fs::read_file( "assets/html/minigame/signal.html" );
    ...
    node_t minigame_generic( ptr_t<string_t> emojis ) { return node_t([=]( ptr_t<node_t> self ){
        ...
        auto list= queue_t<ptr_t<float>>(); auto tmp = ptr_t<uchar64>( 0UL );
        auto uif = fs::read_file( "assets/html/minigame/generic.html" );
        ...
        for( int x=10; x-->0; ){ 
            ...
            raw[4] = type::cast<float>( rand()%20  )+ 20.;
        list.push(raw); } 
    ...
    node_t minigame_motor() { return node_t([=]( ptr_t<node_t> self ){
        ...
        auto uif = fs::read_file( "assets/html/minigame/temperature.html" );
        auto tmp = ptr_t<uchar64>( 0UL );
        ptr_t<uint> val ({ rand()%100, rand()%100 });
        ...
        self->onNext ([=](){ 
            if( key::is_pressed( rl::KEY_ESCAPE ) ){ self->free(); } 
            if( key::is_pressed( 'A' ) ){ process::call( addr, EM_VAL(3) ); }
            if( key::is_pressed( 'D' ) ){ process::call( addr, EM_VAL(4) ); }
        });
    ...
    node_t minigame_clasification(){ return node_t([=]( ptr_t<node_t> self ){
        ...
        auto uif = fs::read_file( "assets/html/minigame/clasification.html" );
        auto tmp = ptr_t<uchar64>( 0UL );
    ...
    node_t minigame_circle(){ return node_t([=]( ptr_t<node_t> self ){
        ...
        auto uif = fs::read_file( "assets/html/minigame/circle.html" );
        auto tmp = ptr_t<uchar64>( 0UL );
    ...
    node_t minigame_data(){ return node_t([=]( ptr_t<node_t> self ){
        ...
        auto uif = fs::read_file( "assets/html/minigame/download.html" );
        auto tmp = ptr_t<uchar64>( 0UL ); auto time = 30; //rand()%10+10;
        ...
        coBegin
            ...
            coDelay( time * 1000 ); EM_EVAL( _STRING_( 
                document.querySelector( '[_${1}_pow]' ).innerHTML = `
                    <button onclick="${0}._nodepp_invoke_( '${1}', 1 )" value="DONE" DONE
                            class="uk-button uk-button-success uk-h1 uk-width-1-1" 
    ...
    node_t minigame_shield(){ return node_t([=]( ptr_t<node_t> self ){
        ...
        auto uif = fs::read_file( "assets/html/minigame/shield.html" );
        auto tmp = ptr_t<uchar64>( 0UL );
    ...
    node_t minigame_number(){ return node_t([=]( ptr_t<node_t> self ){
        ...
        auto uif = fs::read_file( "assets/html/minigame/number.html" );
        auto pss = ptr_t<string_t>( 0UL ); 
        auto tmp = ptr_t<uchar64>( 0UL );
        ...
        auto val = raw.as<EM_VAL>(); switch( val.as<int>() ){
            ...
            *pss += tmp; } while(0); break;
            ...
            case 11: do { pss->clear(); } while(0); break;
            case  0: do { self->free(); } while(0); break;
            case 10: do { 
    ...
    node_t minigame_med   (){ return minigame_generic( ptr_t<string_t>({ "🦠" }) ); }
